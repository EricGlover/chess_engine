...
use crate::move_generator::{gen_pseudo_legal_moves, Move, Pin};
...
impl<'a> AttackMap<'a> {
    ...
    fn add_moves(&mut self, color: Color, moves: &Vec<Move>, board: &'a dyn BoardTrait) {
        moves.iter().for_each(|m| {
            ...
            match color {
                Color::White => info
                    .attacking_white_pieces
                    .push(board.get_piece_at(&m.from).unwrap()),
                Color::Black => info
                    .attacking_black_pieces
                    .push(board.get_piece_at(&m.from).unwrap()),
            }
        });
    }
...
#[cfg(test)]
mod tests {
    use crate::board::{BoardTrait, Coordinate};
    ...
    //         let attacker_found = info
    //             .attacking_white_pieces
    //             .iter()
    //             .any(|&piece| piece == attacker);
    //         println!("{:?}", info);
    //         println!("{:?}", attacker);
